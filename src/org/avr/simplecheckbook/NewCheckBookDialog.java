...
public class NewCheckBookDialog extends Dialog<MasterCheckBook> {
	
	Optional<MasterCheckBook> result = null;
	
	private TextField txtDbName = new TextField();
	 ...
	 */
	public NewCheckBookDialog(Stage primaryStg) {
		...
		this.setResultConverter( new Callback<ButtonType, MasterCheckBook>() {
			...
			public MasterCheckBook call(ButtonType param) {
				if (param == btnOk) {
					...
					mcb.setDbLocation( dirLocation.getAbsolutePath() );
...
//					mcb.setStartingBalance( new BigDecimal(txtBeginingBalance.getText()) );
	...
	private boolean validEntries() {
		...
		if (txtDbName.getText() == null || txtDbName.getText().trim().length() == 0 ) {
			errorMessages.append("- Missing DB Name \n");
		}
		if (dirLocation == null) {
